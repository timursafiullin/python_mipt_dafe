...
class RegressorLSM(RegressorABC):
    ...
    def fit(self, abscissa: Sequence[Real], ordinates: Sequence[Real]) -> None:
        ...
        # average() из numpy
        x_avg = self._average(self._abscissa)
        y_avg = self._average(self._ordinates)
        x_squares_avg = self._average([x**2 for x in self._abscissa])
        xy_avg = self._average([x*y for x, y in zip(self._abscissa, self._ordinates)])
    ...
    @staticmethod
    def _average(sequence: Sequence[Real]) -> float:
